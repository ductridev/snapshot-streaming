...
object SnapshotService {
  ...
  def make[F[_]: Async: KryoSerializer: SecurityProvider](
    ...
    val nodeClients = config.nodeUrls.map(url => NodeClient.make[F](client, Uri.unsafeFromString(url)))
  ...
  def make[F[_]: Async, T](
    ...
    new SnapshotService[F] {
      ...
      def downloadAndSendSnapshots(init: ProcessedSnapshots) = for {
        snapshot <- nodeService.getSnapshots(init.lastProcessedOrdinal, init.gaps)
      ...
      val updateProcessedSnapshots: (ProcessedSnapshots, Long) => ProcessedSnapshots = (previousState, ordinal) =>
        previousState match {
          case ProcessedSnapshots(lastProcessedOrdinal, gaps) if ordinal > lastProcessedOrdinal =>
            ProcessedSnapshots(ordinal, ((lastProcessedOrdinal + 1) until ordinal).toList ::: gaps)
